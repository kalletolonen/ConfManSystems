...
The current maturity state is 1.0 in the finished repo.
...
After this I needed to create and start the  environment.
...
I didn't need salt-master for this project by default, but I installed it anyway on the same go. I opened a fresh terminal in my home direcotory and entered the following commands.
...
First I created a folder for my project and put a "helloworld" for Salt in it.
...
Virtualenv is an [isolated copy of Python](https://www.pythonforbeginners.com/basics/how-to-use-python-virtualenv), so deleting some directories propably damaged the system quite badly.
...
The command "ls" did actually prove that it did not work at all, eventhough the stdout suggested it did.
...
I remembered from earlier [exercices](https://github.com/kalletolonen/ConfManSystems/blob/main/h5.md) that I could have a requirements.txt in my salt-server's file location, so I added those lines to my state and a requirements.txt to /srv/salt/djangoadmin/
...
The error told me that the Virtual Environment wasn't propably working. The state produced some different errors too.
...
I realized that the earlier env-activation would have ended before the next command, so I just added that to a new command.
...
On a second thought, this wasn't actually helpful, since I didn't have the debug-tools of the development server at my use (Salt was just executing endlessly (ie. running the server)). On this note I omitted this command from my state and opted to do the migrations instead.
...
Django does have a CLI-command to create a superuser without user input, so I decided to try it out. My main problem was that ./manage.py was producing not producing stdout. It was printing stderr instead and, to my knowledge, you can't use grep on that. Django does not allow setting a superuser from CLI with a password without human input[0](https://vuyisile.com/how-to-automate-creating-a-django-super-user/), so I googled for a workaround ([1](https://code.djangoproject.com/ticket/27769),[2](https://unix.stackexchange.com/questions/117467/how-to-permanently-set-environmental-variables)).
...
Salt had ran as root and thus the user rights were not optimal.
...
I concluded that there was no way for me to know what user the user of this Salt state would like to use it for, so I would make it easy to customize the state with Jinja variables.
...
The second thing I thought about was making the setup just a tad more secure, so I decided to add a custom settings.py for the project and allow access only from localhost, so that my state wouldn't be implemented to production without atleast modifying that file manually.
...
Things I had chosen to not implement on my state:
- A default model (since I have no clue about what users would be developing)
...
I had some errors that were a result of trying to do something with a file that hadn't yet been created. My updated final state was as follows:
